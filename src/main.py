...
from src.commands import routed_root_command, commands_hash_map
...
def absolutely_all_handler(message):
    ...
    if current_route.route in commands_hash_map:
        return commands_hash_map[current_route.route].run(
            message=message,
            bot=bot,
            database=database,
    ...
    # TODO: что делаем с пользователем у которого current_route - некорректен?
    return routed_root_command.run(
        message=message,
        bot=bot,
        database=database,
        current_route=current_route,
